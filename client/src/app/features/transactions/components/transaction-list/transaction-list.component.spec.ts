...
describe('TransactionListComponent', () => {
  ...
  describe('Accessibility', () => {
    ...
    it('should have aria-label on checkboxes', () => {
      ...
      const headerCheckboxInput = checkboxes[0].query(By.css('input'));
      // Assuming default loader returns the key
      expect(headerCheckboxInput.nativeElement.getAttribute('aria-label')).toBe('TRANSACTIONS.SELECT_ALL');

      // Row checkboxes start from index 1
      const rowCheckboxInput = checkboxes[1].query(By.css('input'));
      expect(rowCheckboxInput.nativeElement.getAttribute('aria-label')).toBe('TRANSACTIONS.SELECT_ROW');
    });
  });
