...
export class TransactionStatisticsService {
  async getDashboardStatistics(
    ...
    // We use queryRaw for time-series aggregation to avoid fetching thousands of rows
    const [allTimeStats, monthlyStats, entertainmentStats] = await Promise.all([
      // 1. All time stats (existing)
      prisma.transaction.groupBy({
        by: ["type"],
        where: { userId },
        _sum: { amount: true },
      }),
      // 2. Monthly stats (Optimized: replaced findMany with aggregation)
      prisma.$queryRaw<Array<{ month: Date; type: string; total: Prisma.Decimal }>>`
        ...
        GROUP BY 1, 2
      `,
      // 3. Entertainment stats (Optimized: replaced findMany iteration with specific query)
    ...
    for (const stat of monthlyStats) {
      // stat.month is a Date object (if driver handles it) or string.
      const txDate = new Date(stat.month);
      const idx =
        (txDate.getFullYear() - rangeStart.getFullYear()) * 12 +
        (txDate.getMonth() - rangeStart.getMonth());
    ...
    if (entertainmentStats.length > 0) {
      const totalEnt = entertainmentStats.reduce((acc, stat) => acc + decimalToNumber(stat.total), 0);
      entertainmentAvg = totalEnt / entertainmentStats.length;
    }
