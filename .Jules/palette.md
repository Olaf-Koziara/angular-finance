...
## 2026-02-03 - [Accessibility] Mat-Checkbox Aria Labels
**Learning:** Angular Material `mat-checkbox` requires `[aria-label]` input rather than `[attr.aria-label]`, and testing requires querying the internal `input` element.
**Action:** Use `[aria-label]` binding for `mat-checkbox` and verify via `query(By.css('input'))` in tests.
